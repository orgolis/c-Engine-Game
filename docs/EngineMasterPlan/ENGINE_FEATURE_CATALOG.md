...
|---|---|---|---|
| **F1** | **Make ECS authoritative** — retire the OOP shadow; inspector/gizmo/save write ECS; gameplay off `GetComponent<T>()` *(Stage 1 remaining)* | Gameplay = components + systems. Today the ECS is a per-frame **shadow** of the OOP scene (OOP is authoritative). G0 attributes/abilities/effects must live in a real authoritative ECS, or every system is built on sand. | 🟡 shadow only |
| **F2** | **Editor inspector → core reflection** *(Stage 0 remaining)* | A new gameplay component (Attribute, Ability, Inventory…) should auto-appear + be editable with **no hand-written editor UI per component** — the line between data-driven and hand-coded. | 🔴 own reflection |
