...
| Deferred G-buffer + PBR materials | ✅ | Metal-rough, 5 maps. **Tiers 1-2 of the material programme (v0.7.6-v0.7.12)** added UV scale/offset, **detail maps** with a distance fade, **triplanar** projection, and **parallax occlusion mapping**. Set 0 of the scene pipeline was an *empty* layout — created only so the material could sit at set 1, destroyed on the next line — and is now a real per-frame UBO; POM and the detail fade both need a camera position, and the vertex push block is at Vulkan's 128-byte floor with no room for one. See [material-system-architecture](../superpowers/specs/2026-08-24-material-system-architecture.md) |
| Forward / transparent (WBOIT) | ✅ | |
| Shadows (cascaded) | ✅ | 3 cascades + 3×3 PCF + point-light shadows, and a ray-traced shadow path alongside (`rtShadowEnabled`) — note the two are **either/or**, not blended: with RT shadows on, `directionalShadow()` returns before the shadow map is ever sampled. Confirmed running at ~0.11 ms/frame. **v0.7.3 fixed shadows sitting wrong on terrain:** the shadow pass draws one LOD tier coarser than the G-buffer, which is a sound saving for decimated meshes and wrong for a heightfield — a terrain tier is a *different surface*, not a simplified copy, so the shadow map held depth for a shape that was not the one being shaded. Terrain now draws the same tier it renders at — **correct, but on a dead path**: with RT shadows on, that function returns before the shadow map is read, so it fixed nothing anyone was seeing. **v0.7.5 found the real cause:** BLAS built from `mesh->index_count()`, the *whole* index buffer, which for terrain holds **every LOD tier concatenated** — 35,520 indices against 26,112 visible, so 1.4x the geometry existed for rays to hit and the coarse tiers sit metres above the ground they approximate. BLAS is now built per-submesh from LOD 0 alone. The clue was "only with RT shadows on"; a shadow-map bug cannot be RT-only. **Softness (v0.7.7):** PCF radius on the map, a sun-disc cone on the ray path — a single ray is binary and has no penumbra at all, so RT shadows had a hard edge by construction. Virtual shadow maps 🔴 |
...
| Asset browser (full-project, drag-drop, import-into-project) | ✅ | **v0.7.13** made it behave like a file manager rather than a viewer: rename, duplicate, cut/copy/paste and Properties, on both the item menu and a right-click on empty space, with F2 / Ctrl+C / Ctrl+X / Ctrl+V / Ctrl+D / Del / Alt+Enter. Two rules the operations hold to — **nothing is ever overwritten** (where Explorer offers replace/skip, this uniquifies: a surprising name is recoverable, a replaced file is not), and names are validated against **Windows** rules before touching the disk, because those failures are silent — `CON.txt` is a reserved device name that fails to create *without erroring*, and a trailing space is trimmed so the rename appears to do nothing. Logic is ImGui-free in `asset_file_ops.h`; `assetops_check` (73) asserts the refusals as hard as the successes, and the guards were mutation-tested to prove they were not vacuous |
| Asset selection → Inspector | ✅ **v0.7.13.** Clicking a file showed it nowhere: the Inspector kept saying *No entity selected* while the file sat highlighted two panels away. An asset now shows name, type, path, size (recursive for folders), modified time and read-only state, plus a text preview for text assets — a `.mat` or `.vfx` is a few lines, and reading them beats a round trip through an external editor. **Entity and asset are one selection between them**, Unity-style, so the panel never has to guess which the user meant; the swap is *detected* once per frame rather than pushed from the ten places that assign an entity id. Everything shown is captured once on selection — a folder's size means walking its whole subtree, which is not a per-frame cost. Text is detected by scanning for a NUL rather than by extension, so an unknown text format still previews and a binary `.dat` does not |
| **Prefabs** (drag an entity out, drag it back in) | ✅ **v0.7.14.** Drag any hierarchy entity onto the asset browser and it is written as a `.prefab` with **every descendant and every property**; drag it back onto the hierarchy to instantiate — onto an entity to parent it there, onto empty space to land at the scene root. **A prefab is a scene file holding one subtree**, not a new format: the same entity writer and the same entity reader, so a component gaining a field reaches prefabs with no extra work and, more usefully, *cannot reach one and not the other*. Two rules make that safe — the root's `PARENT_ID` is **suppressed** (kept, it names whatever the entity was parented to in the scene it came from, which resolves to nothing today and to a *stranger* the day that id exists in the target scene), and instantiated entities get **fresh ids**, so one prefab placed twice does not produce two entities claiming one id. The ECS-side `Prefab` in `prefab.h` is a different thing and stays: it captures authorable *gameplay* components for spawning, not a scene subtree. `prefab_check` (37), with the PARENT_ID suppression mutation-tested |
| **Creating and editing a material** | ✅ **v0.7.15** — and the gap it closed was the whole point of Tiers 1–2. Every control for editing a material existed and **none of them could be reached without an object that already had one**: the panel keys off a `MeshRenderer`, so a material not yet on anything was unreachable, and a *terrain layer* material was unreachable **permanently** — terrain carries no MeshRenderer at all. There was also no way to create a `.mat` from anywhere: the New menu offered a folder, three script kinds, an item table and a text file. So the answer to “how do I put a texture on the ground” was that there was no door. Now: **New › Material** writes one through the format's *own* writer (so a new field cannot appear in the format and be missing from every material the menu creates — asserted in `material_check`), and selecting a `.mat` in the browser opens the **real editor** in the Inspector — Surface, Textures, Detail, Parallax, UV — rather than a text dump |
| **Starter textures** | ✅ **v0.7.15.** A new project contains no texture, so every path for applying one worked and there was nothing to apply. **New › Texture** generates a **Checker**, a **Grid** or a **UV chart** as a 24-bit BMP — no encoder, no dependency, and stb_image (what the engine loads through) reads it. The checker earns its place specifically for **reflections**: a flat colour reflects a flat colour, so a mirror and a matte surface look identical against one, and neither shade is pure black because a black cell has no diffuse response and would carry no lighting information at all. `texgen_check` (31) asserts the *pattern*, not just the header — a generator emitting a flat fill produces a perfectly valid image that teaches nothing — plus the 4-byte row padding, which 512 hides (512×3 is already a multiple of 4) and which shears the image diagonally rather than failing when wrong. Every pattern was decoded back through stb_image to confirm it loads |
| **Drag an asset onto an object** | ✅ **v0.7.16.** Drop a `.mat`, a texture, a script, a mesh, a sound, an `.hdr` or a `.prefab` onto an object — in the **hierarchy**, or in the **viewport onto the object under the cursor**. Before this, nearly every drop target was a specific named field in the Inspector, so the gesture only worked if you already knew which panel, which section and which text box. The rule table lives in `asset_drop.h`, apart from any panel, because a drop can land in several places and if each decided for itself they would disagree — a texture that sets the albedo in one place and silently does nothing in another is not a bug anyone reports clearly. **Every decision carries a message, including the refusals**: a drop that does nothing and says nothing is indistinguishable from a drop that missed. `assetdrop_check` (48) asserts that no combination of kind and target is ever silent |
|  ↳ the three rules worth knowing | An **`.hdr` ignores what it landed on** and sets the scene sky — it is an environment, not a surface, and there is exactly one thing a person means by dragging a sky into a scene. A **texture on an object using a shared material is applied, not refused, and the message names how many other objects changed** — silently repainting twenty objects is the one outcome nobody predicts; with no `.mat` assigned it sets the object's own albedo instead, which cannot surprise anyone. And the **viewport applies to the object under the cursor, not to the selection** — a drag is aimed with the mouse, and using the selection is the one behaviour that guarantees the wrong object gets the texture. Picking is the *same* `PickEntityAt` the click path uses, shared rather than duplicated, so a drop and a click at one pixel always resolve to one object |
| **Moving files in the browser** | ✅ **v0.7.17.** Drag a file or a whole folder onto another folder — a tile, a list row, or a node in the left tree. The tree matters more than it looks: the file pane shows only the current folder's children, so without it the only direction a drag could go is *deeper*. Moves run through the same `assetops::paste` that cut/paste uses rather than calling rename directly, so a drag inherits every guard that path already has — no overwriting, no folder into its own subtree, no move into the folder a file already occupies. A drag is the gesture most likely to land somewhere unintended, so it is the one that needs those guards most. `assetops_check` grew to **81**, including that a moved folder brings its *nested* children (a top-level-only move reports success and arrives empty) and that a name collision uniquifies rather than **merging** two folders |
| **Editing a prefab in the Inspector** | ✅ **v0.7.17.** Select a `.prefab` and it is inspected as the **object it is** — transform, components, Add Component, everything the entity inspector does — rather than as a file with a text preview. Nothing in the ~1,600-line inspector needed changing: it reads a scene and an entity id once at the top, so a **staging scene** holding the prefab supplies different ones and the rest runs unchanged. The stage is deliberately *not* the live scene — instantiating into the real level to edit would leave an object the user never placed and has to remember to remove. A **Parts** list picks which entity in the subtree is being edited, without which a prefab with children could only ever have its root edited. Saves on gesture end, not per frame: a slider held for two seconds would otherwise rewrite the file 120 times. **Baked, not linked** — objects already placed from a prefab are separate copies and do not follow, which the banner says out loud |
|  ↳ the hundred call sites | The inspector calls `MarkModified()` from over a hundred places. Editing a *prefab* through it would raise a false “unsaved scene” prompt from every one of them, for a file the scene does not contain. Rather than audit a hundred call sites for a condition none of them can see, an RAII guard snapshots the flag and restores it — RAII specifically because `ShowInspector` returns from several places, and a guard that only ran on the last one would leave the scene falsely dirty on all the others. The same guard is what detects that an edit happened at all |
|  ↳ fixed in passing | Selecting an asset printed **“Entity not found”** underneath the asset panel, from v0.7.13 until this release: control fell through the asset branch into the entity lookup with id 0. Cosmetic, and exactly the kind of thing headless checks cannot see |
| **Audio mix buses + mixer panel** | ✅ **v0.8.0.** `Window › Audio Mixer`: a fader, mute, solo and a post-fader meter per bus, over a tree of Master + Music/SFX/UI/Voice/Ambience. **The plan said this rode an existing bus graph; there was none** — `grep -rn "[Bb]us"` over the audio tree returned nothing, the mixer summed all 256 voices straight to output, so the routing layer had to be built first and the panel is the visible tenth. Gain folds to the root through an `effective_gain` walk **bounded by `kMaxBuses`**, because a parent cycle must cost a wrong number rather than a hung audio callback. `mix()` stays allocation- and lock-free: per-bus scratch is sized once at `init()`, longer blocks are chunked, and fader moves cross to the audio thread on the existing command queue. `audiobus_check` (33) |
|  ↳ why solo is the hard half | "Solo = mute every other bus" silences the **children of the group you just soloed**, which reads as "solo is broken" rather than as a routing bug. Audibility therefore asks whether the bus *or any ancestor* is soloed. Mutating it back to the naive form fails exactly one assertion — the one written for it |
|  ↳ why voices sum per bus | Pre-scaling each voice by its bus gain produces **identical audio** and no meter: a bus's level is the level of the *sum* of its voices, and once they are mixed into master there is nothing left to measure. The per-bus accumulation is also where a per-bus effect goes later |
|  ↳ the reachable path | A mixer whose faders control nothing assignable is the v0.7.15 failure repeated, so `AudioSourceComponent` gained a bus **by name** — an index would silently re-route every already-authored scene the day a bus is inserted — serialized as `AUDIO_SRC_BUS`, chosen from an Inspector dropdown that warns when a scene names a bus the project no longer has. Layout persists per **project**, not per scene, and saves on **gesture end** so a held fader does not rewrite the file 120 times |
| **User-extensible editor** | ✅ **v0.8.0.** Drop a script in `<project>/editor_scripts/` and it adds commands to the editor: they appear in the palette (Ctrl+P) and in `Window › Extensions`, and the file hot-reloads on save. Works in **all three backends** — Python, C++ and C#. **This is the promise `command_palette.h` made in 4.2** ("an agent or a script can drive the editor without a bespoke hook per feature"), true about the design and false about the product until now. An extension can also `run_command("Save Scene")` — anything the editor does is reachable by name |
|  ↳ why not ScriptSystem | That loop is per-**entity** and **play-mode-only**: it instantiates when an entity is first seen while playing and destroys everything on Stop. An extension has no entity and must live in **edit** mode. `ExtensionSystem` is a sibling sharing only the three `ScriptHost` backends — but an extension is still an **ordinary script** using the same `on_start(e)` hook with entity 0, so no backend needed a new load path, only `ScriptInstance::invoke` |
|  ↳ a token, not a callback | A function pointer back into a pocketpy VM or a .NET assembly is not expressible across one C ABI for three languages. `register_command(title, category, token)` names a no-argument function every backend can already resolve: a module global, a `GetProcAddress`, a method name |
|  ↳ the invisible bug it is built around | Reload re-runs `on_start`, which registers again. Without `CommandRegistry::owner` and a removal that happens **before** the re-run, every save appends a duplicate of every command — and each duplicate still *works*, so nothing looks broken until the palette is unusable. `extension_check` (53) asserts three reloads leave one copy; deleting the guard fails exactly four assertions |
|  ↳ what the fake host could not see | The loader is tested through a **fake** backend, correctly — its behaviour is not any language's business. Forty-six assertions passed. Adding one group that loads a real `.py` failed **seven at once**, including the shipped starter template: the bindings live in the `engine` module like every other script binding here, and the template called `register_command(...)` bare instead of `import engine` / `engine.register_command(...)`. The first thing any user would try did not run. The template is now asserted to load under real Python |
| **The authoring documents persist** | ✅ **v0.8.1.** `MaterialGraph`, `AnimGraph` and `Sequence` were built, reachable, and each had its own passing test suite — and **none of them could be saved**. A material graph or a cutscene existed only until the editor closed, and no panel said so. Now `.matgraph`, `.animgraph` and `.seq` are real files: **New ▸ Authoring** creates one (there was previously no way to create any of the three at all), double-click opens it in its own editor, and a shared document bar shows the path, a dirty marker and Save. Conventions come **wholesale from `vfx_io.h`** — version stamp, every field written including defaults, unknown keys ignored, a parse that always succeeds — rather than three formats argued out separately. `docio_check` (70) |
|  ↳ names, not indices | Enums are written by NAME. An index in a file is a promise that nobody will ever reorder the enum, and the day someone inserts a `NodeKind` every saved graph silently becomes a different graph — no error, because every index still parses. The same reasoning put a bus *name* into `AudioSourceComponent` one release earlier: **a file should name what it means, not where it happened to sit** |
|  ↳ ids are remapped, not trusted | Documents are rebuilt on load through their own `add_*` API, so the internal id counter stays ahead of everything loaded. Writing ids straight into the vectors would leave the counter at 1, and the next node the user added would collide with one from the file |
|  ↳ the bug the check was written for | `kAnyState` is `0xFFFFFFFF`, and `std::stoi` throws `out_of_range` on it — so an int-based parse quietly returned the fallback and **every "interrupt from anywhere" transition** (hit reactions, deaths, teleports) loaded as from-state 0, matched no state, and was dropped. No error, no warning; the character would simply never play its death animation again. The assertion existed before the bug, because Any-State was the obvious silent case |
|  ↳ why a round trip of defaults proves nothing | A test that builds a default document and compares it to itself passes against a writer that emits **nothing** — both sides are default. Item 3.7 found emitters and NPC agents were never written to the scene file at all, and it survived exactly because nothing asserted a non-default value. So every field here is set to a distinctive non-default value first, and the strongest assertions are behavioural: identical generated GLSL, identical evaluation across the timeline, identical validation output |
|  ↳ last release's lesson, applied at once | v0.8.0 shipped an extension template that could not run because its test built its own copy. Here the starter documents live in `doc_io` as functions, the Asset Browser calls them, and the check loads **those** — one definition, two callers. The check also asserts the starter material graph generates working GLSL rather than merely parsing |
...
| Lumen-grade GI | 🔴 |
| Hair / skin / material layering | 🔴 — Tier 3 of the material programme; needs Foundation B (shading-model IDs) first |
| **Bindless textures** | ✅ **v0.7.8** — capability query, slot allocator, descriptor table (`UPDATE_AFTER_BIND` + `PARTIALLY_BOUND` + variable count, 4096 slots). Gated on real device support with `GWS_FORCE_NO_BINDLESS` to test the fallback. **Built, not yet consumed**: the first consumer is terrain, whose 4-layer ceiling is a descriptor-count limit rather than a design one |
| Contact shadows | ✅ |
...
| Terrain LOD | ✅ **v0.6.9+.** Power-of-two index tiers over the same vertex buffer — a coarser tier is just an index list visiting every Nth vertex, so tiers cost index data and nothing else. 8704 / 2304 / 640 / 192 triangles per 64-cell chunk at 0 / 100 / 200 / 400 m (45x at the coarsest). Skirts cover the cracks between neighbouring tiers; holes are honoured at every tier so caves never seal over with distance. Required fixing LOD distance to use the mesh bounding-box centre rather than the model origin — every chunk of a terrain shares one transform, so origin distance gave them all the same tier. See [`TERRAIN_RENDER_PERF.md`](../architecture/TERRAIN_RENDER_PERF.md) |
| Per-layer PBR materials | ✅ **v0.6.9.** A layer references a `.mat` and gets albedo + normal + metal/rough plus its own tint and PBR factors, on terrain's **own 14-binding** descriptor layout and pipeline. Before this a layer was one albedo texture and a tiling number, with roughness hardcoded at 0.95 for every square metre of every terrain — the splat shader reinterpreted the standard 6-binding material layout as `[splat, 4× albedo]`, and five samplers cannot hold four layers × three maps. A layer with no material still falls back to its single texture at roughness 0.95, so upgrading does not restyle an existing project's ground. Tiling stays on the component: one rock repeats 8× across a 100 m terrain and 300× across a 4 km one. **v0.7.6** made a layer's metallic / roughness / normal scale editable — the fallback path had them as *hard-coded constants*, so a terrain without `.mat` layers was permanently non-reflective whatever the inspector showed. **v0.7.9** added triplanar projection: UV-tiled ground stretches into visible smears on any slope steeper than gentle, because a heightfield's UVs are a top-down plan, and cliffs are exactly where terrain is looked at |
