...
> engine plan (Pillars A–O) onto the _actual current state_ of the codebase, ordered by what must exist
> before what, with implementation steps and the working theory of each subsystem.
>
> **Target game:** _Project Schizo_ — open-world multiplayer action-RPG / looter-shooter
...
The original AAA plan groups work into **pillars A–O**. Pillars are _categories_, not an execution order.
...
- **Current state** — what is actually in the repo today (verified agai
  nst source, not docs).
...
> **Honesty note on "done":** the renderer rework checklist marks many boxes ✅ that reflect _intent_.
...
| 8 | World | 🟡 verified **and now integrated** (`1c12b74`): camera-driven cell residency + floating-origin rebase on the live scene, Tools toggle | world_check (21/21) | async job load of real partitioned content (a cell's content is currently the entities inside it); HLOD; material/anim LOD; procedural gen |
...
**Repo health (2026-08-09):** ✅ **clean clones now build** — fixed the same day (issue #2). Testing a real clone
found four independently fatal problems, all the same class: undeclared submodule gitlinks, our ufbx build glue
living *inside* a submodule, the tinyusdz flag patch existing only in a working tree, and zstd's entire source
tree hidden by an unanchored `lib/` ignore rule. Verified end to end. **Branches also resolved the same day**
(issue #3): the old `main` turned out to be the abandoned OpenGL lineage with an *unrelated history*, now archived
as `legacy-opengl-2026-04`, with the working branch promoted to `main`. ⚠️ Still open: CI only triggers on `v*`
tags, so its health is unobservable between releases — that blind spot is what let the clone breakage survive
twenty releases. Detail in [`REMAINING_WORK.md`](REMAINING_WORK.md) §Repo & release health.
...
| H Networking      | 🟡 Transport done; Stage 7 in progress     | The prior "rollback + deterministic sim + input sync" core is mostly **scaffold** (the `DeterministicSimulation` fixed-step driver has an empty `ProcessFixedTick`; `RollbackManager` has snapshot/restore structure; `NetworkManager` is a prediction-focused API with a **stubbed** `ServerConnection`). **Added 2026-06-15 (Stage 7 step 1 — transport):** vendored **ENet** (`third_party/enet`, builds on MinGW: winmm/ws2_32) behind a backend-agnostic `Transport` interface (`engine/core/network/transport.h` + `enet_transport.cpp`): server/client, `connect`/`send`/`broadcast`/`poll`, Reliable + Unreliable channels, `PeerId` mapping. **Verified** (`tools/net_check`): in-process ENet server+client over loopback UDP — connect handshake + bidirectional reliable PING/PONG. **Added 2026-06-15 (step 2 — replication):** `replication.h` — NetId-keyed snapshots whose component bytes come from the reflection archive (`gws::serialize`, single source of truth); `ReplicationServer.broadcast(world, tick)` + `ReplicationClient.update(world)` (find-or-create client entities by NetId, apply components). **Verified** (`tools/repl_check`): a server ECS world syncs 3 NetId+Transform entities to an initially-empty client world over the transport, positions match, and moving a server entity mirrors to the client. **Added 2026-06-15 (step 3 — prediction + reconciliation):** `prediction.h` — `PredictionClient` applies input immediately (predict) + buffers it; on an authoritative snapshot it snaps + re-simulates the unacked inputs (reconcile). `PredictionServer` is authoritative (processes buffered inputs, acks). The sim step is a pluggable deterministic `SimStep` (wraps the deterministic Jolt fixed-step later). **Verified** (`tools/prediction_check`, simulated latency): converges (no drift), responsive (client leads server), smooth (reconcile adds no rubber-band when prediction is right), and corrects to the authoritative path when the server diverges (a wall) — at 6- and 20-tick latency. Remaining (multi-session): headless server target, interest management, delta-encoding + snapshot interpolation, and wiring the SimStep to physics + the packets onto the live Transport.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
1. **Jobs and ECS are retrofit-hostile.** Adding multithreaded job scheduling and a data-oriented ECS _after_
...
2. **Networking is retrofit-hostile** (the design doc says so explicitly). The deterministic core already
   exists; extend it into replication _before_ gameplay systems proliferate state (Stage 7, but its
...
- An **allocator** is an object that owns a region of memory and hands out sub-regions with a known
  lifetime policy. The engine standard is: _no system calls the OS allocator directly; it requests an
  allocator appropriate to the lifetime of the data._
...
- **Frame allocator:** an arena that is `reset()` once per frame, _double-buffered_ so frame N's transient
...
- A **work-stealing thread pool**: one worker thread per hardware core minus reserved threads (render
  ...
  own jobs from one end (LIFO, cache-hot); idle workers **steal** from the _other_ end of a random victim's
...
- **Fibers (optional, later):** instead of blocking a worker on `wait`, switch to another fiber so the OS
  ...
  spin-help model.
- **Determinism hook (for netcode):** the _simulation_ job graph must produce identical results regardless
...
Stand up the _instrumentation API_ now (the full UI is Stage 14) so every later stage is born instrumented:
...
- **Sparse-set (EnTT):** each component type has its own packed array + a sparse index `entity→slot`.
  Iterating one component is cache-perfect; iterating _combinations_ (a "view") hops between arrays. Cheap
  add/remove. Excellent default, battle-tested, header-only.
- **Archetype (Unity DOTS / Unreal Mass style):** entities with the _same set_ of component types live
...
layer** that compiles down to ECS components, _or_ is replaced outright. Lowest-risk path: keep editor-facing
...
assets. The renderer already consumes glTF at runtime; that becomes the _import_ front-end of a real cook
pipeline.
...
The renderer is the strongest pillar. This stage finishes the _breadth_ the AAA plan implies. Each item slots
...
- **Broad phase** (find _possible_ collisions cheaply): an AABB tree / SAP over body bounds → candidate
...
- **Determinism:** Jolt is deterministic given identical inputs/order — essential because networking (Stage 7) rolls back and re-simulates physics. Run physics inside the deterministic fixed-step (existing
...
> cube falls 4.0→0.54 onto the ground in play mode; default scene seeded with a Player + 2 cubes + a dynamic
> mesh-collider object + ground so Play just works). Dynamic mesh colliders use a **convex-hull** body
> (`add_convex_body`) since Jolt triangle meshes are static-only; static mesh colliders keep the accurate
> triangle mesh (verified: a dynamic mesh object falls 5.5→0.55 via its convex hull). The legacy custom `PhysicsWorld` is fully unused now. Remaining (not acceptance): step 5
> hitbox system (rides Stage 10 combat), step 6 cloth/ropes/destruction (E4, deferred), and a hands-on pass on
> character-movement _feel_ (the one thing a headless check can't judge).
...
_asset_, IK, motion matching, animation streaming. The game needs root-motion, parkour, and frame-data
combat poses.
...
- **F4 IK:** **two-bone IK** (analytic) for foot/hand placement; **FABRIK** or CCD for chains; foot IK
  raycasts to ground and adjusts ankle + pelvis so feet plant on uneven terrain. Applied as a _post-pass_
...
# STAGE 6 — Audio (Pillar G) — **acceptance MET; bus graph/reverb/DSP/streaming remain**

> **STATUS 2026-06-15 — core spatial audio system built + verified; audible in-editor.**
> Backend: **miniaudio** (vendored `third_party/miniaudio`, impl compiled once) + a **custom
> mixer** (the recommended path). New core lib **`gws_audio`** (`engine/core/audio/`):
>
> - **G1 mixer** ✅ — device-agnostic `Mixer`: fixed 256-voice pool summed per block, gain +
>   pitch/resample + looping, **lock-free SPSC `CommandQueue`** (game→audio thread, no locks/
>   alloc in `mix()`), master gain. `AudioEngine` opens a miniaudio device whose real-time
>   callback pulls from the mixer; `init_offline()` for headless/tests. Clip decode (WAV/FLAC/
>   MP3) via miniaudio.
> - **G2 spatial** ✅ — `compute_spatial()`: linear distance attenuation to `radius`,
>   constant-power stereo pan from listener azimuth, Doppler from relative radial velocity.
> - **ECS bridge** ✅ — `AudioScene` drives the engine from `AudioSource` + a new `AudioListener`
>   component each frame (clip-by-GUID, live position/volume, velocity from position deltas).
> - **Editor** ✅ — a spatial test tone (synth blip) plays at a world point; the listener follows
>   the camera (View → Audio to toggle). **Audible — pending the user's ears.**
> - **G2 occlusion** ✅ — `PhysicsWorld::raycast()` (Jolt `NarrowPhaseQuery::CastRay`) added; the
>   `AudioScene` occlusion hook (a pluggable raycast callback, so the bridge has no physics dep)
>   sets a per-voice occlusion 0..1; the mixer applies a per-voice one-pole **lowpass + level
>   duck**, smoothed per-sample (click-free). Editor: a static occluder wall + camera→tone
>   raycast each frame muffles the tone when blocked (logs OCCLUDED/CLEAR). **Audible test.**
>
> Verified headless by **`tools/audio_check`** (ALL OK): exact gain/sum/pitch/loop/stop, 128
> concurrent voices, lock-free queue, pan L/R/center + distance + Doppler, the ECS bridge
> start/stop/move, and occlusion (clear rms 0.50 → occluded 0.008, ~36 dB; bridge hook muffles a
> blocked source). **Acceptance MET** (128 concurrent spatial voices, lock-free zero-alloc mix =
> no audio-thread stalls, occlusion filters blocked sources) — pending the user's audible confirm.
> **Beyond core:** G1 bus graph, G3 reverb zones, G4 DSP inserts, G5 streaming, G6 procedural.
...
1. Pick a backend: **integrate FMOD/Wwise** (fast, AAA-grade, but licensing) _or_ build on **miniaudio**
...
fighting-game-style lockstep. The MMO-looter-shooter design needs this _extended_ into
...
> **STATUS 2026-06-15 — IN PROGRESS (multi-session stage).** The pre-existing rollback/deterministic-sim/
> input-sync core is mostly scaffold (empty `ProcessFixedTick`, stubbed `ServerConnection`). **Step 1
> transport DONE:** ENet vendored + a backend-agnostic `Transport` (reliable/unreliable channels, server/
> client, connect/send/broadcast/poll) — verified by `tools/net_check` (loopback connect + bidirectional
> reliable PING/PONG). **Step 2 replication DONE:** `replication.h` (NetId-keyed, reflection-driven snapshots;
> `ReplicationServer`/`ReplicationClient`) — verified by `tools/repl_check` (server ECS world syncs 3
...
- **I4 Large-world coordinates:** floating-point precision breaks past a few km from origin. Use a
  **camera-relative / floating-origin** scheme (rebase the world so the camera is near origin; render in
  ...
  a _large seamless_ world + space/dimension travel in the lore.
...
Most of these _exist in skeleton form_ (`core/character`, `core/ability`, `core/gameplay`, `core/network`)
...
>   sorted + colored per tag. **Live data.**
> - **N2 GPU** ✅ _(pending visual confirm)_ — `engine::vulkan::GPUProfiler` compiled, and the
>   render graph's already-resolved per-stage timestamps now FEED it: `GPUProfiler::submit_frame()`
>   - filled `VulkanRenderGraph::update_gpu_profiler()`; the editor calls `resolve_timings()` +
>     `update_gpu_profiler()` after the frame fence (same point it resolves occlusion queries). The
>     render graph already wrote `vkCmdWriteTimestamp` around every stage — only the sink was a stub.
>     Shows real Shadow/Geometry/Lighting/Transparent/PostProcess ms on timestamp-capable GPUs;
>     needs a hands-on run to confirm the numbers populate.
> - **N3 Memory** ✅ — `memory_snapshot.h` (per-tag live/peak from the A1 `MemoryTracker`) +
>   an opt-in `AllocatorRegistry` (enumerable allocator used/reserved/**fragmentation**); the
>   editor's double-buffered frame allocator is registered → live occupancy bar. **Live data.**
> - **N4 Network** ✅ — `Transport::stats()` (per-channel bytes/packets, RTT, loss) +
>   `NetProfiler` (bandwidth rates) + prediction **rollback depth**. Offline in a plain editor
>   session; lights up when a transport is live (Stage 7 integration). **Verified headless** by
>   `tools/profiler_check` (counters/bandwidth/rollback all exact).
> - **N5 Frame capture** ✅ — `gws::profile::FrameCapture` (core, header-only): arm-once →
>   snapshot one frame's per-stage draw list (Geometry/Transparent/Shadow: mesh id, submesh,
>   vertex/index counts, blend flag) → overlay "Frame Capture" section (Capture button + the
>   draw list + Save .txt export) + an spdlog summary line. **Verified headless** by
>   `profiler_check` (lifecycle/counts/triangle-total/re-capture). The in-house lightweight
>   counterpart to a RenderDoc capture (submission list + counts, no resource bytes).
>
> All five pillars build; the unified overlay covers them. Acceptance ("hot path visible in one
> overlay") MET for CPU+Memory+GPU+capture (GPU + the whole overlay pending a hands-on visual
> confirm) with Net wired (offline in a plain editor session). Remaining: captured-baseline
> regression gating (compare a saved capture/timings against a budget in CI).

Not a phase — instrument from Stage 0 onward; this stage _formalizes_ the tools.
...
_Document version 1.0 — 2026-06-12. Maps Pillars A–O onto the live codebase in dependency order._
_Update the current-state table (§1) as stages land; keep the dependency graph (§2) authoritative for order._
