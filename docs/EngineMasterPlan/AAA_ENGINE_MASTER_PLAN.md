...
tree hidden by an unanchored `lib/` ignore rule. Verified end to end. ⚠️ Still open: the default branch `main` is
**117 commits behind** the working branch, and CI only triggers on `v*` tags so its health is unobservable
between releases. Detail in [`REMAINING_WORK.md`](REMAINING_WORK.md) §Repo & release health.
