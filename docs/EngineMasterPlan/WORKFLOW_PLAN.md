...
| 4.6 | Animation state-machine graph editor | engine | **Unblocked**: `NodeCanvas` (`76166f2`) is the reusable surface this needed. The plan assumed the logic-graph panel already provided it; measured, that file is 255 lines welded to `LogicNodeKind`. Porting the logic graph onto the new canvas would also **prove** the reuse claim, which currently has exactly one consumer |
