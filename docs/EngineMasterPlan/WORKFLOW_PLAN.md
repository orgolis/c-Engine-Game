...
| 3.7 | Editor inspector + scene save onto core reflection/serialization | engine | 🔴 **Blocked by ordering, not by difficulty — do 3.6 first.** Measured: **no OOP scene component is reflected** (zero `GWS_REFLECT` in `engine/scene/include`), and all 13 keep private members behind accessors, while `gws::reflect` is **offset-based** and needs public standard-layout fields. So this means exposing 13 component types or building property reflection — **and 3.6 deletes those very components**. Doing it first is work 3.6 throws away. Second half also needs a decision: core serialization is **binary**, the scene format is text, and #58 wants semantic scene diffs — going binary trades that away |
