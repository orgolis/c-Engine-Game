...
| 2.4 | **Hot reload everything else** — shaders, textures, meshes, scenes, component definitions | engine | 🟡 **Partly done** (`d0af2a0`). The three ad-hoc mtime loops (textures, scripts, sky) are replaced by one shared `AssetWatcher` that **settles** — mtime *and* size must hold steady before firing, so a file still being written by Blender is never read half-finished. That was a latent bug in all three copies. **Meshes reload now** (the cache was keyed by path and never invalidated). **Shader iteration went 17.7 s → ~1.2 s** (`4602a03`): the renderer prefers a `.spv` on disk over the baked array, and `gws shaders` compiles into that directory. That is a restart, not live reload, and it currently reaches **2 shaders** — most passes keep their GLSL inline in the renderer `.cpp` files, which is #67 and the prerequisite for going further. Left: **scenes**, which need #18's diff-and-choose rather than a blind reload that could destroy in-editor work |
...
| 2.6 | Never block the editor thread — import/cook/bake with progress + cancel | engine | 🟡 **Infrastructure done** (`b2ad0c1`). Premise verified: cooking `assets/models` takes **15.9 s**. `gws_jobs`' JobSystem turned out to be the wrong tool — it is fork-join, so a task outliving hundreds of frames would either block the caller or starve frame work. `TaskRunner` owns a small pool instead, and **completion callbacks run on the editor thread** so a finished import may legally touch the GPU and the scene. Drag-drop import is the first user: chunked, cancellable, and it deletes the partial file if cancelled. 35 assertions, run 15× without flakes. **Mesh parsing now runs off-thread for OBJ** (`bb1585e`): parse on a worker, GPU build in the completion callback, both sharing one `build_obj_scene` so an async mesh is byte-identical to a sync one. glTF stays synchronous — its loader interleaves parsing with device calls. `--frames N` was added to verify it on a GPU, and immediately exposed two pre-existing bugs: `resolve_asset_path` could not drop a **stale leading `../`**, so the default scene mesh had been silently falling back to a primitive, and the async error log did not say *why* a load failed |
| 2.7 | Undo covering gameplay components, terrain, logic graph, script/agent edits | engine | 🟡 **Surveyed and two defects fixed** (`dc34dd6`). The survey mattered more than the coverage: undo covered only 12 creates + rename, and **the creates were wrong** — they undid by `GetEntityByName`, so with two same-named entities undo deleted the *first*, and after a rename it silently did nothing. Undo that removes the wrong object is worse than none. Now by id. **Deletion is undoable at last** — it was not, on any of the three paths — and it was cheap: `RemoveEntity` only unregisters, so the command's `shared_ptr` keeps the entity alive and `AddEntity` is the exact inverse. `undo_check`: 19 assertions. **Inspector field edits are undoable** (`ca7b20f`), coalesced to **one entry per drag**. The coalescing rule is a pure ImGui-free state machine (`edit_coalescer.h`) precisely because its hard cases are gestures — dragging back to the start records nothing, a checkbox commits in one frame, switching fields mid-drag commits the first edit — and none of those can be checked by reading code or running the editor headlessly. 26 assertions cover them. Undo is a deserialize of a stored blob, so it is generic over the authorable registry. **Gizmo drags and component add/remove are undoable too** (`62691c6`). The gizmo needed no coalescing — it already has explicit BeginDrag/EndDrag, so the gesture boundaries are stated rather than deduced; a click that selects without moving records nothing. Component removal now serializes the component *before* destroying it, and `apply_component_bytes` treats an empty blob as "should not exist", so **one command shape covers edit, add and remove** rather than three that could disagree. **Still uncovered:** terrain sculpt, logic-graph edits, reparenting, applying play-mode changes |
