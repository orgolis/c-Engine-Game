...
| 3.7 | Editor inspector + scene save onto core reflection/serialization | engine | 🟡 **Tractable half done** (`4f719e6`). The Phase-3 components are plain public-field structs by design, so `SkinnedMesh`, `ParticleEmitter`, `NpcAgent` and `Mesh` are reflected now — adding a field used to mean **four** hand edits with a silent failure mode. Guarded by `static_assert` on `sizeof`, because a runtime check cannot see a field that was never registered (the first version of that test was tautological and passed when I added one). **Left:** the older components keep private members behind accessors and offset reflection needs public fields — and the save half needs a **decision**: core serialization is binary, the scene format is text, and #58 wants semantic scene diffs |
