...
| 3.2 | **Enable the GPU-driven path** | engine | 🔴 **Premise corrected — the flag was the least of it.** `gpu_driven_enabled` lived on `IndirectDispatcher`, which was **not referenced by anything, not compiled** (commented out of CMakeLists), and not what the HZB culler used — the live `VulkanHzbCuller` has its own indirect buffers. So this was never "flip a flag": the class the flag belonged to was dead code, deleted in the 3.10 sweep. What remains is real work: route draw submission through `VulkanHzbCuller`'s existing indirect buffer. **Deferred on verification grounds**, same as the VFX GPU pass — a render path can be API-correct, pass validation and draw nothing, and confirming otherwise needs a human looking at pixels |
| 3.3 | ~~**World-streaming integration**~~ | engine | ✅ **Done** (`1c12b74`). Camera-driven cell residency + floating-origin rebase on the live scene. A cell's content is the entities inside it (activate/deactivate) — genuine residency behaviour without inventing a partitioned content format that would be thrown away later. **The rebase contract is honoured explicitly**: `update()` *returns* the shift rather than moving the camera itself, because the editor owns the camera — entities-but-not-camera makes the world teleport, camera-but-not-entities makes the camera fly away. Roots only; children inherit. Off by default with a Tools toggle, and switching off restores everything |
...
| 3.5 | ~~AI perception + NPC behaviour trees + combat AI~~ | engine | ✅ **Perception + behaviour + navmesh movement done** — **pulled forward** from its planned position because 3.4 and 3.8 were finishing as green rows that nothing consumed. Behaviour trees, the blackboard, the path follower and the vision cone were each verified and driven by nothing; `NpcAgentComponent` joins them. Tree is `Selector[ Sequence[can-see → chase], patrol ]`, so chasing pre-empts patrolling every tick. **Paths come from the baked navmesh**, and the check asserts the agent does **not** move at all without one — a straight-line slide toward the target would look correct and prove nothing. `npcagent_check`: 12 assertions on observable behaviour, because the failure mode is an agent that ticks happily and never moves. Combat AI (target → ability → execute) is **not** done |
...
| 3.10 | **Remove or salvage ~7,000 lines of dead pre-ECS code** | engine | 🟡 **3,182 lines removed** — the whole `gpu/vulkan/` Phase-6 editor/debug cluster: `ui_manager`, `debug_panels`, `scene_hierarchy_panel`, `inspector_panel`, `profiler`, `profiler_panels`, `debug_visualization`, `hzb_culler`, `indirect_dispatcher`, `transform_gizmo`, `query_pool`, `gpu_profiler_panels`. All were **commented out of the build** under "pending GCC compat fixes" and superseded by editor-side versions. Two were worse than clutter: `profiler.h` and `transform_gizmo.h` **shadowed live headers of the same name** in other directories — an include-path hazard, not just dead weight. Proven dead before deletion: not compiled, not included by anything live, and the cluster only referenced itself. **Left:** the legacy `engine/core/{gameplay,ability,character}` libraries (~5,800 lines) — those ARE linked by the editor, so "dead" has to be proven per symbol, and the plan wants the character library's input-buffer + locomotion *salvaged* rather than dropped |
