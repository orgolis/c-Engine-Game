...
| Hot reload everything else — shaders, textures, meshes, scenes, component definitions | 🟡 | Principle: **nothing requires a restart; treat every restart as a bug.** **Textures and meshes reload now** (`d0af2a0`) on one shared `AssetWatcher` that replaced three ad-hoc mtime loops — and *settles* before firing, so a file still being written by Blender is never read half-finished, a latent bug all three copies had. **Shaders take ~1.2 s via a restart** (`4602a03`, 11 of them since `607c5d9`) but are not yet live. **Scenes are deliberately not done**: a blind reload of a scene edited externally could destroy in-editor work, so it needs #18's diff-and-choose treatment. Component definitions are C++ and only reachable through the script/DLL path. |
...
| Never block the editor thread | 🟡 | **Done for import and OBJ mesh parsing** (`b2ad0c1`, `bb1585e`), with progress and a cancel button. `gws_jobs` turned out to be the wrong tool — it is fork-join, so a task outliving hundreds of frames would either block the caller or starve frame work; `TaskRunner` owns a small pool instead, and **completion callbacks run on the editor thread** so a finished import may legally touch the GPU and the scene. Shader compilation is moot (nothing compiles GLSL at runtime, and `gws shaders` is offline). **Left: glTF**, whose loader interleaves parsing with device calls, and navmesh bake. |
| Undo covering *everything* | 🟡 | Partial undo is worse than none: it teaches people not to rely on it — and part of this one was **actively wrong**, undoing creates by `GetEntityByName` so it deleted the wrong entity when two shared a name (`dc34dd6`). Now covered, one entry per gesture: entity create/delete/rename, **inspector field edits, gizmo drags, component add/remove** (`ca7b20f`, `62691c6`). The coalescing rule is a separate ImGui-free state machine because its hard cases are *gestures* — dragging back to the start records nothing, a checkbox commits in one frame — which cannot be verified by reading code. **Left:** terrain sculpt (wants a storage strategy; naive heightmap copies are large), logic-graph edits, reparenting, applying play-mode changes. |
