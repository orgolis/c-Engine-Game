...
| # | Finding | Detail |
|---|---|---|
| **R1** | 🔴 **Engine CI has never passed** — and the same cause breaks clean clones | All 20 release runs failed in 11–27 s with `fatal: No url found for submodule path 'third_party/JoltPhysics' in .gitmodules`. **Four dependencies — `JoltPhysics`, `enet`, `tinyusdz`, `ufbx` — are committed as submodule gitlinks (mode 160000) but never declared in `.gitmodules`**, so `git submodule update --init` cannot resolve a URL. A fresh clone gets four empty directories and fails at `CMakeLists.txt:120` (`add_subdirectory(third_party/ufbx)`). Every published release was built and uploaded **by hand**. **Fix = four `.gitmodules` entries.** (Hub CI is green by contrast.) |
