...
**v0.8.5.** Usable and under active development by a small team.
...
| Tests | 68 check binaries run through `gws test`, on Windows and Linux CI |
...
**Shipping** — **Project → Export Game…** packages the open project as a playable
game: a portable folder and `.zip` on Windows, a folder and a single `.run` file on
Linux. See [`docs/EXPORTING_A_LINUX_GAME.md`](docs/EXPORTING_A_LINUX_GAME.md).
...
**Run the checks** — from the repository root; several checks locate assets relative
to it.
...
(`shadersource_check`, `terrainmat_check`); three more need a GPU and are skipped
unless you pass `--gpu`, because CI runners have none.

### Building from source on Linux

**Requirements** — the same packages CI installs on Ubuntu 24.04:
...
plus a Vulkan driver for your GPU — on Ubuntu, `mesa-vulkan-drivers` covers Intel
and AMD.

```sh
git clone --recurse-submodules https://github.com/orgolis/c-Engine-Game.git
cd c-Engine-Game
cmake --preset linux-debug          # once
cmake --build build/linux-debug --target editor --parallel
./build/linux-debug/bin/editor
```

Run the checks with `./build/linux-debug/bin/gws test`, from the repository root.
...
**Where files go** — settings and recent projects in `~/.config/gameworldshaper`, the
shader pipeline cache in `~/.cache/gameworldshaper`, logs and crash reports in
`~/.local/state/gameworldshaper/diagnostics`. Each respects its `XDG_*_HOME`
override. (On Windows: `%APPDATA%\GameWorldshaper` and
`%LOCALAPPDATA%\GameWorldshaper`.)
...
hot-reloads when you save it. The same works in C++ and C# on Windows. Use
...
- **Every behavioural change ships with a check.** There are 68 of them and they run
...
- **Green CI is not the whole suite.** Three checks need a GPU and CI runners have
  none, so a change to the renderer or device setup should be checked locally with
  `gws test --gpu` before it merges.
- **Runtime files do not belong in the repository.** Settings, caches, logs and crash
  reports go to per-user directories (see [Linux](#linux)); if one appears in
  `git status`, something resolved a path against the working directory.
