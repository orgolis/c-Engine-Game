...
std::unique_ptr<VulkanSsaoPass> VulkanSsaoPass::create(VulkanDevice* device,
    ...
    // Default to the best available technique: RT if the GPU supports it,
    // otherwise SSAO. The editor can switch at runtime via set_technique().
    pass->technique_ = (use_rt && pass->rt_available_) ? AoTechnique::RT
                                                        : AoTechnique::SSAO;
...
bool VulkanSsaoPass::create_pipeline() {
    ...
    li.bindingCount = (technique_ == AoTechnique::RT) ? 5u : 4u;
    li.pBindings    = b.data();
    if (vkCreateDescriptorSetLayout(vk, &li, nullptr, &dsl_) != VK_SUCCESS) return false;

    // Push: mat4 view_proj + ivec2 output_size + 3 floats + pad = 96B
...
void VulkanSsaoPass::execute(VkCommandBuffer cmd,
    ...
    // RT AO uses a long ray length (enclosure detection). HBAO/HDAO/GTAO
    // work in world space too and want a medium radius; plain SSAO uses the
    // small config radius (it has its own internal far group).
    switch (technique_) {
        case AoTechnique::RT:   pc.radius = 30.0f;          break;
        case AoTechnique::HBAO:
        case AoTechnique::HDAO:
        case AoTechnique::GTAO: pc.radius = 3.0f;           break;
        default:                pc.radius = config_.radius; break;
    }
    ...
    if (blur_pipeline_ != VK_NULL_HANDLE) {
        ...
        bpc.depth_sigma  = 0.01f;  // tolerate small depth jitter
        bpc.normal_sigma = 0.20f;  // tolerate ~25° angle differences
        bpc.radius       = 2;      // 5x5 kernel
